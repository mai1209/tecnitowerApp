import { publishMqttRawCommand } from "../mqtt/mqttListener.js";
...
function extractResponseFrame(buffer, { unitId, functionCode }) {
  ...
  for (let index = 0; index <= response.length - 5; index += 1) {
    ...
    if (frame.length === length && hasValidCrc(frame)) {
      return frame;
    }
  }

  throw new Error(`Respuesta Modbus MQTT inválida: ${response.toString("hex")}`);
}
...
async function readRawRegisterViaMqtt({ elfinId, unitId, register }) {
  const request = buildReadHoldingRegisterFrame({ unitId, register });
  const response = await publishMqttRawCommand(elfinId, request);
  const frame = extractResponseFrame(response, { unitId, functionCode: 0x03 });
  assertNoModbusException(frame);

  if (frame[2] !== 2) {
    throw new Error(`Respuesta Modbus MQTT inesperada para registro ${register}`);
  }

  return toSigned16(frame.readUInt16BE(3));
}

async function writeHoldingRegisterViaMqtt({
  elfinId,
  unitId,
  register,
  rawValue,
  ...
  const sendWrite = async (resolvedFunctionCode) => {
    ...
    const response = await publishMqttRawCommand(elfinId, request);
    const frame = extractResponseFrame(response, { unitId, functionCode: numericFunctionCode });
    assertNoModbusException(frame);
...
export async function writeRegisterValueViaElfinMqtt(
  value,
  {
    elfinId,
    unitId,
  ...
  if (!elfinId) throw new Error("Falta elfinId para MQTT directo al Elfin");
  ...
  const rawBeforeVerify = Number.isFinite(numericVerifyRegister)
    ? await readRawRegisterViaMqtt({ elfinId, unitId: numericUnitId, register: numericVerifyRegister })
    : null;

  if (debugLabel) {
    console.log(`🧪 [${debugLabel} MQTT RAW]`, {
      stage: "verify-before",
  ...
  const meta = await writeHoldingRegisterViaMqtt({
    elfinId,
